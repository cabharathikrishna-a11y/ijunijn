...
object DevicePresenceManager {
            ...
            if (snapshot != null && snapshot.exists()) {
                ...
                for (deviceSnapshot in snapshot.children) {
                    ...
                    if (lastUpdateDate == todayStr || lastUpdateDate == yesterdayStr || lastUpdateDate == tomorrowStr) {
                        val otherTodayMs = deviceSnapshot.child("Todays_Focus_Ms").getValue(Long::class.java)
                            ?: deviceSnapshot.child("todayFocusMs").getValue(Long::class.java)
                            ?: ((deviceSnapshot.child("Todays_Total_Focus_Seconds").getValue(Long::class.java) ?: 0L) * 1000L)
    ...
    suspend fun updateDeviceFocusStats(context: Context, email: String) {
        ...
        try {
            ...
            // Adopt highest today focus from other devices using the FRESH localTodayFocusMs (completed history)
            ...
            if (dbUrl.isNotEmpty()) {
                ...
                val finalTodayFocusSecs = finalTodayFocusMs / 1000L
                val formattedTodayFocusTime = TimelineSyncEngine.formatTimeMsToHhMmSs(finalTodayFocusMs)

                val statsUpdates = mapOf<String, Any>(
                    "Todays_Total_Focus_Seconds" to finalTodayFocusSecs,
                    "Todays_Total_Focus_Time" to formattedTodayFocusTime,
