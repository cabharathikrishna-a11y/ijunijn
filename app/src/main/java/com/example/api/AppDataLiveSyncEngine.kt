...
object AppDataLiveSyncEngine {
    ...
    fun startListening(context: Context, email: String, database: AppDatabase) {
        ...
        try {
            ...
            val hListener = object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    CoroutineScope(Dispatchers.IO).launch {
                        reconcileHealth(database, snapshot)
                    }
                }
                override fun onCancelled(error: DatabaseError) {}
            }
            userRef.child("HEALTH_LIVE").addValueEventListener(hListener)
            healthListener = hListener
    ...
    fun stopListening(context: Context) {
        ...
        try {
            ...
            if (dbUrl.isNotEmpty() && activeUserEmail.isNotBlank()) {
                val rtdb = FirebaseDatabase.getInstance(dbUrl)
                val userRef = rtdb.getReference("FOCUS_TIMMER").child("USER").child(activeUserEmail)
                ...
                healthListener?.let { userRef.child("HEALTH_LIVE").removeEventListener(it) }
    ...
    fun pullAllDataFromCloud(context: Context, email: String, database: AppDatabase) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                userRef.child("HEALTH_LIVE").get().addOnSuccessListener { snapshot ->
                    CoroutineScope(Dispatchers.IO).launch { reconcileHealth(database, snapshot) }
                }
    ...
    fun pushHealthToCloud(context: Context, email: String, record: HealthRecord) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val rtdb = FirebaseDatabase.getInstance(dbUrl)
                val key = record.dateString
                val ref = rtdb.getReference("FOCUS_TIMMER")
                    .child("USER").child(sanitizedEmail)
                    .child("HEALTH_LIVE").child(key)

                val payload = mapOf(
                    "dateString" to record.dateString,
                    "steps" to record.steps,
                    "stepGoal" to record.stepGoal,
                    "sleepMinutes" to record.sleepMinutes,
                    "sleepGoalMinutes" to record.sleepGoalMinutes,
                    "waterMl" to record.waterMl,
                    "waterGoalMl" to record.waterGoalMl,
                    "caloriesBurned" to record.caloriesBurned,
                    "calorieGoal" to record.calorieGoal,
                    "activeMinutes" to record.activeMinutes,
                    "activeMinutesGoal" to record.activeMinutesGoal,
                    "heartRateAvg" to record.heartRateAvg,
                    "breakfastFoods" to record.breakfastFoods,
                    "lunchFoods" to record.lunchFoods,
                    "dinnerFoods" to record.dinnerFoods,
                    "snacksFoods" to record.snacksFoods,
                    "timestamp" to System.currentTimeMillis()
                )
                ref.setValue(payload)
            } catch (e: Exception) {
                Log.e(TAG, "Failed to push Health to RTDB", e)
            }
        }
    }
